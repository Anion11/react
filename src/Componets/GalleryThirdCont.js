...
function Imgs(props) {
    ...
    const spawnResProp = (e) => {
        changeImgs(ThirdBild1,ThirdBild2, ThirdBild3 , e.target); 
        setSpawnAnim(true);
        const timer = setTimeout(() => {setSpawnAnim(false); clearTimeout(timer)},300);

    }
    const spawnComrProp = (e) => {
        changeImgs(ThirdBuildComr,ThirdBuildComr1, ThirdBuildComr2, e.target);
        setSpawnAnim(true);
        const timer =  setTimeout(() => {setSpawnAnim(false); clearTimeout(timer)},300)

    }
    const spawnAgrProp = (e) => {
        changeImgs(ThirdBuildingAgr,ThirdBuildingAgr1, ThirdBuildingAgr2,e.target);
        setSpawnAnim(true); 
        const timer =  setTimeout(() => {setSpawnAnim(false);  clearTimeout(timer)},300);

    }
    const spawnIndProp = (e) => {
        changeImgs(ThirdBuildingInd,ThirdBuildingInd1, ThirdBuildingInd2,e.target);
        setSpawnAnim(true); 
        const timer = setTimeout(() => {setSpawnAnim(false);  clearTimeout(timer)},300)
    ...
    return(
        <div className="galleryPhoto">
            <div className="namesHouse">
                <div className="activeLinkSecond" onClick={spawnResProp}>Residential Property</div>
                <div className="activeLinkSecond" onClick={spawnComrProp}>Commercial Property</div>
                <div className="activeLinkSecond" onClick={spawnAgrProp}>Agriculture Property</div>
                <div className="activeLinkSecond" onClick={spawnIndProp}>Industrial Property</div>
            </div>
            <CSSTransition
                in={spawnAnim}
                nodeRef={nodeRef1}
